...
"""[1:-1].split('\n') #初めの改行をカット
        ...
        actual = replace_lib.remove_negation_from_ikemasen(token_lines)
        expected = ['走り はしり 走る 動詞 * * * 子音動詞ラ行 * 基本連用形 * "代表表記:走る/はしる" <代表表記:走る/はしる><正規化代表表記:走る/はしる><文頭><かな漢字><活用語><自立><内容語><タグ単位始><文節始><文節主辞>', 'ましょう ましょう ます 接尾辞 14 動詞性接尾辞 7 動詞性接尾辞ます型 31 意志形 4 "代表表記:ます/ます"']
        self.assertEquals(actual, expected)
...
"""[1:-1].split('\n') #初めの改行をカット
    ...
    def test_remove_negation_from_naide_kudasai4(self):
        token_lines = """
